...
if( !class_exists('Infetech_Woo') ){
	...
	class Infetech_Woo {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_single_product_image_thumbnail_html', array( $this, 'infetech_single_product_image_thumbnail_html' ), 10, 2 );
		...
		function infetech_woocommerce_show_title_shop_page( $param ){
			...
			if( is_shop() && get_post_meta( infetech_get_current_id(), "infetech_meta_page_heading", true ) != 'no' ){
		...
		function infetech_single_product_image_thumbnail_html( $html, $attachment_id ){
			...
			if ( $attachment_id ) {
				...
				$html  = '<div class="woocommerce-product-gallery__image--placeholder">';
					$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image"  />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'infetech' ) );
					$html .= '</div>';
			}
			return $html;
